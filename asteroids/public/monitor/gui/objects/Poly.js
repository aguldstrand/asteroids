define(function() {
	...
	Poly.setFocusPoint = function(fp) {
		Poly.fp = fp;
	};
	...
	Poly.addRS = function(originX, originY, x1, y1, x2, y2, x3, y3, polys, scale) {
		polys.push(
	...
	Poly.addR = function(originX, originY, x1, y1, x2, y2, x3, y3, polys, scale) {
		polys.push(
			x1 * scale + originX - Poly.fp.x, y1 * scale + originY - Poly.fp.y,
			x2 * scale + originX - Poly.fp.x, y2 * scale + originY - Poly.fp.y,
			x3 * scale + originX - Poly.fp.x, y3 * scale + originY - Poly.fp.y);
		return 3;
	};
	...
	Poly.add = function(x, y, width, height, polys, rotation, part, originX, originY) {
		...
		if (rotation) {
			...
			var x1 = x + 0 - Poly.fp.x;
			var x2 = x1 + width;
			var y1 = y + 0 - Poly.fp.y;
			var y2 = y1 + height;
	...
	Poly.addS = function(x, y, width, height, polys, rotation, part, originX, originY) {
